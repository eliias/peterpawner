...
func DebugAttacks(list []uint8) string {
	...
	for i := 0; i < 64; i += 1 {
		...
		if chess.Contains(list, uint8(i)) {
			str += "1"
		} else {
			str += "0"
		}
...
func DebugGame(game chess.Game) string {
	var str = "Game:\n"
...
func DebugMoves(board []uint8, depth int, color uint8, enpassant []uint8) string {
	var list = generator.Generate(board, color, enpassant)
...
func DebugMove(move *chess.Move) string {
